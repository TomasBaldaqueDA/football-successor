...
export async function POST(req: NextRequest) {
  ...
  const target = body as {
    ...
    /** Filter candidates by player_dim.age_last_season (inclusive). Omit or null = no bound. */
    min_age?: number | null;
    max_age?: number | null;
  };
  ...
  try {
    const { rows: l4lRows } = await pool.query(
      `select * from mart.l4l_neighbors($1::bigint, $2::text, $3::int, $4::text, $5::int, $6::int)`,
    ...
    const candidateIds = Array.isArray(l4lRows)
      ? l4lRows.map((r: { player_id: unknown }) => String(r.player_id))
      : [];
    ...
    function metricSlice(merged: Record<string, unknown>): Record<string, number | null> {
      ...
      for (const col of chosen) {
        out[col] = pickJsonNumber(merged, col);
      }
      return out;
    }
    ...
    const rows =
      Array.isArray(l4lRows) && l4lRows.length > 0
        ? l4lRows.map((r: Record<string, unknown>) => {
