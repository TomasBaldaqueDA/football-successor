...
export async function POST(req: NextRequest) {
  ...
  const target = body as {
    ...
    /** How many weighted metrics to show (default 8, max 12) */
    comparison_metrics?: number;
    /** Filter candidates by player_dim.age_last_season (inclusive). */
    min_age?: number | null;
    max_age?: number | null;
  };
  ...
  let minAge: number | null =
    target.min_age === undefined || target.min_age === null ? null : Number(target.min_age);
  let maxAge: number | null =
    target.max_age === undefined || target.max_age === null ? null : Number(target.max_age);
  if (minAge !== null && !Number.isFinite(minAge)) {
    return NextResponse.json({ error: "min_age must be a number" }, { status: 400 });
  }
  if (maxAge !== null && !Number.isFinite(maxAge)) {
    return NextResponse.json({ error: "max_age must be a number" }, { status: 400 });
  }
  if (minAge !== null) minAge = Math.round(minAge);
  if (maxAge !== null) maxAge = Math.round(maxAge);
  ...
  try {
    // role_neighbors filters:
    // - candidates share selected_bucket in player_position_membership
    // - optional age bounds by player_dim.age_last_season
    const { rows: roleRows } = await pool.query(
      `select *
       from mart.role_neighbors($1::bigint, $2::text, $3::int, $4::text, $5::int, $6::int)`,
      [String(targetPlayerId), selectedBucket, topN, weightVersion, minAge, maxAge],
    );

    const targetIdStr = String(targetPlayerId);
    const candidateIds = Array.isArray(roleRows)
      ? roleRows.map((r: { player_id: unknown }) => String(r.player_id))
      : [];
    ...
    for (const r of mergedRows) {
      mergedById.set(r.player_id, asRecord(r.j));
    }
    ...
    const rows = (Array.isArray(roleRows) ? roleRows : []).map((r: Record<string, unknown>) => {
